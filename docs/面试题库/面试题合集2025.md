...
- MVC 模式 : Backbone + underscore + jquery
  ...
  不管是视图从模型中获取数据去渲染，还是用户输入通过控制器更新模型然后再更新视图，控制器承担的责任太大了，不利于维护。

- MVVM 模式 : 映射关系的简化 （隐藏 controller）

​		MVVM模式是一种通过ViewModel将数据模型与用户界面连接起来，实现两者分离的设计模式。ViewModel的主要任务是管理数据流和业务逻辑，确保视图和模型保持同步，同时尽量减少两者之间的直接依赖，让开发人员能够更专注于业务逻辑和数据处理，而不是界面细节。
