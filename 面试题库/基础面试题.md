...
- 比如说我向你发送一封电子邮件， 标题很吸引人。
- 但邮件正文正隐藏着`<img src=xxx.com/>pay?id=200>`
...
- 当我们打开网页时，浏览器先去请求对应的 HTML 文件。HTML 只是字符串，计算机不识别，网络传输内容都是`0`和`1`这样的**字节数据**。浏览器会先把**字节数据转换为字符串**。
- 然后将字符串**语法分析**转换为**标记**（token）
- 标记化后紧接着转换为**Node**
- 然后根据 Node 之间的联系构建成**DOM 树**。

##### 将 CSS 文件转换为 CSSOM 树

- 和转化 DOM 树基本相似，浏览器会**先确定每个节点的样式**，样式可以自行设置，也是可以继承的。
...
![事件循环](https://gitee.com/l544402029/res/raw/master/小书匠/1588134366471.png)
